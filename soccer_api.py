...
def _arkivo_ndeshje(pred, ht_str=None):
    ...
    rec = {
        ...
        # ── A U PUBLIKUA si premium/PPM (kriteri i historikut PPM; kap edhe 1-1 që s'është 'value bet') ──
        "is_premium":    pred.get("is_premium"),
...
def _arkivo_sweep():
    ...
    try:
        r = requests.get(
            ...
            f"koef_1,koef_x,koef_2,odds_reale,rezultati_sakt,tregjet,best_bet,besueshmeria,training_data,dist_gola,is_value,is_premium,rezultati"
...
# ── PESHAT E BURIMEVE TË xG (env-var, të akordueshme pa deploy) ──
#    Saktësia e matur e secilit sinjal për ANËN FITUESE (100 ndeshje jo-barazim, korrik 2026):
#      tregu 72%  |  forma 55%  |  Elo 53%
#    Forma NUK bëhet më e saktë kur hendeku i formës rritet (54–56% te të tri brezat) —
#    nënshkrim i zhurmës, jo i informacionit. Prandaj pesha e saj u ul nga 0.40 në 0.25
#    dhe tregu u ngrit nga 0.25 në 0.40 (peshat pasqyrojnë saktësinë e matur).
#    Peshat NORMALIZOHEN automatikisht që shuma të jetë 1 — pra vetëm RAPORTI mes tyre ka
#    rëndësi, dhe s'ka rrezik që ndryshimi i njërës të zhvendosë shkallën e xG-së.
#    Kthimi te gjendja e vjetër: W_FORMA=0.40, W_MARKET=0.25.
def _peshe_env(emri: str, parazgjedhje: float) -> float:
    try:
        v = float(os.environ.get(emri, str(parazgjedhje)).strip())
        return v if v >= 0 else parazgjedhje
    except Exception:
        return parazgjedhje
...
if _w_sum > 0:
    ...
    W_FORMA, W_ELO, W_MARKET, W_BASE = (0.25, 0.25, 0.40, 0.10)
...
def llogarit_xg_te_perparuara(
    ...
    Parazgjedhja tani: 40% tregu + 25% forma + 25% Elo + 10% bazë.
    """
...
# ── CS_LEAN_XG: rregulla CLEAN-SHEET → X-1. Kur skori i zgjedhur ka njërën anë me 0 GOLA
#    dhe ajo anë pritet të shënojë (xG i saj >= CS_LEAN_XG), 0-i zëvendësohet me 1.
#    BAZA (62 pika clean-sheet të mbaruara, me kuotat reale nga odds_reale):
#      • tregu i NËNÇMON skorët X-1 — kuota mes. 10.57 kundrejt 7.10 për X-0
#      • ana e parashikuar me 0 shënon ~53% të rasteve
#      • kthim historik: X-0 +28% vs X-1 +128% (pa 2 goditjet më të mëdha: +86%)
#      • te 31 ndeshjet që MBETEN clean-sheet edhe me normalizimin e ri: -44% vs +111%
#    KUJDES: prova është historike; matet përpara para se të besohet.
#    FIK me CS_LEAN_XG=99 (parazgjedhja). 0.0 = konverto të gjitha. 1.10 = vetëm zona më e sigurt. ──
try:
    CS_LEAN_XG = float(os.environ.get("CS_LEAN_XG", "99").strip())
except Exception:
    CS_LEAN_XG = 99.0
try:
...
def simulim_monte_carlo_v2(
    ...
    # ── RREGULLI CLEAN-SHEET → X-1 (shih CS_LEAN_XG lart) ──
    #    Vepron PAS draw→lean, që të prekë edhe skorët që dalin nga ai rregull.
    #    xg_1/xg_2 këtu janë vlerat e NORMALIZUARA që përdor simulimi (jo ato të loguara).
    #    MBROJTJE: kërkohet fituesi >= 2 gola, që konvertimi të MOS e kthejë fitoren në barazim
    #    (1-0 → 1-1 do të prishte rregullin e fituesit; prova jonë mbulon vetëm X>=2).
    if CS_LEAN_XG < 90.0:
